...
from tfx.examples.experimental import iris_pipeline_sklearn
...
class IrisPipelineSklearnEndToEndTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self._module_file = os.path.join(
        os.path.dirname(__file__), 'iris_utils_sklearn.py')
    self._serving_model_dir = os.path.join(self._test_dir, 'serving_model')
    self._pipeline_root = os.path.join(self._test_dir, 'tfx', 'pipelines',
                                       self._pipeline_name)
    self._metadata_path = os.path.join(self._test_dir, 'tfx', 'metadata',
                                       self._pipeline_name, 'metadata.db')
    self._blessing_path = os.path.join(os.path.dirname(__file__), 'data',
                                       'blessing')
  ...
  def testIrisPipelineSklearn(self):
    BeamDagRunner().run(
